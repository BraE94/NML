	...
	public class PdfApplicationDocumentGenerator : IApplicationDocumentGenerator
	{
		...
		public byte[] Generate(Guid applicationId, string baseUri)
		{
			...
			if (application != null)
			{
				...
				{
					view = AppPending(application, _templatePathProvider, _configuration, View_Generator, baseUri);
				}

				else if (application.State == ApplicationState.Activated)
				{
					view = AppActivated(application, _templatePathProvider, _configuration, View_Generator, baseUri);
				}

				else if (application.State == ApplicationState.InReview)
				{
					view = AppInReview(application, _templatePathProvider, _configuration, View_Generator, baseUri);
				}
				else
				{
					_logger.LogWarning(
